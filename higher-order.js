...
let ordersTotal = orders.map(function(element, i, arr){
  var priceAndTaxTotal = element.price * (element.tax + 1);

  return priceAndTaxTotal;
})
...
var bobsTotal = 0;
bobsPriceArray = purchases.map(function(element, index, arr){
...
for(i=0; i<bobsPriceArray.length; i++){
  bobsTotal += bobsPriceArray[i];

}
console.log(bobsPriceArray)
